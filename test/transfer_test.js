...
import { bridgeAbi } from "./fixtures/bridgeAbi";
...
describe('Aergo Erc20 transfer', function() {
    ...
    before( async () => {
        ...
        account = await aergoWallet.accountManager.addAccount({ chainId: chainId, address: aergoAddress });
    ...
    describe('Ethereum => Aergo', function() {
        it('Should increase approval', async function() {
            this.timeout(100000);
            const receipt = await eta.increaseApproval(web3, bridgeEthAddr, amount, aergoErc20Addr, erc20Abi);
            assert.equal(receipt.status, true)
        });
        it('Should lock tokens', async function() {
            this.timeout(100000);
            const receipt = await eta.lock(web3, receiverAergoAddr, aergoErc20Addr, amount, bridgeEthAddr, bridgeAbi);
            assert.equal(receipt.status, true)
        });
        it('Should build lock proof', async function() {
            this.timeout(100000);
            proof = await eta.buildLockProof(web3, hera, receiverAergoAddr, aergoErc20Addr, bridgeEthAddr, bridgeAergoAddr);
        ...
        it('Should become unfreezeable after anchor', async function() {
            ...
            while (unfreezeable === 0) {
                [unfreezeable, pending] = await eta.unfreezeable(web3, hera, bridgeEthAddr, bridgeAergoAddr, receiverAergoAddr, aergoErc20Addr)
            }
        });
        it('Should unfreeze tokens', async function() {
            this.timeout(100000);
            const txSender = receiverAergoAddr;
            const builtTx = await eta.buildUnfreezeTx(web3, hera, txSender, bridgeEthAddr, bridgeAergoAddr, receiverAergoAddr, aergoErc20Addr);
